...
class Query {
	...
	public function _pre_get_posts_for_search( $query ) {
		if ( $query->is_main_query() && ! is_admin() ) {
			...
			// Taxonomy query.
			$taxonomies = filter_input( INPUT_GET, 'unitone-search-taxonomies', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
			if ( $taxonomies ) {
				$tax_query = array();
			...
			// Meta query.
			$post_metas = filter_input( INPUT_GET, 'unitone-search-post-meta', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
			if ( $post_metas ) {
				$meta_query = array();
			...
			// Period query.
			$period_start = filter_input( INPUT_GET, 'unitone-search-period-start' );
			$period_end   = filter_input( INPUT_GET, 'unitone-search-period-end' );
			...
			do_action_ref_array( 'unitone_search_pre_get_posts', array( &$query ) );
		}
	}
	...
	protected function _includes_future_posts( $post_type ) {
		return apply_filters( 'unitone_search_includes_future_posts', false, $post_type );
	}
	...
	public static function is_search( $query ) {
		return $query->is_search() && null !== filter_input( INPUT_GET, 'unitone-search' );
	}
